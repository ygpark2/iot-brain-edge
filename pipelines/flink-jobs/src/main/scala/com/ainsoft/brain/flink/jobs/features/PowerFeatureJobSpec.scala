...
object PowerFeatureJobSpec extends JobSpec {
  override val name: String = "feature-power"
  ...
  override def register(env: StreamExecutionEnvironment): Unit = {
    ...
    val outputTopic = Env.get("FLINK_FEATURES_POWER_TOPIC", "features-power")
    val groupId = Env.get("FLINK_FEATURES_GROUP_ID", "flink-feature-power")
    ...
    val filtered = env
      ...
      .filter(new FilterFunction[FeatureEvent] {
        override def filter(value: FeatureEvent): Boolean =
          value.sensorType == "VOLT" || value.sensorType == "AMP" || value.sensorType == "WATT" || value.sensorType == "KWH"
      })
    ...
    filtered.sinkTo(sink).name("feature-power-sink")
  }
}
